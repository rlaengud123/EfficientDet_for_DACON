...
def train(opt):
    ...
    optimizer = torch.optim.Adam(model.parameters(), opt.lr, weight_decay=0.05)
    ...
    for epoch in range(opt.num_epochs):
        ...
        if epoch % opt.test_interval == 0:
            ...
            if epoch % 100 == 0:
                print('{} Model Saved Epoch : {}'.format(str(datetime.now())[:19], epoch))
                torch.save(model, os.path.join(opt.saved_path, "signatrix_efficientdet_coco_epoch{}_{}.pth".format(epoch, loss)))
                ...
                if isinstance(model, nn.DataParallel):
                    ...
                    torch.onnx.export(model.module, dummy_input,
                                      os.path.join(opt.saved_path, "signatrix_efficientdet_coco_epoch{}_{}.onnx".format(epoch, loss)),
                                      verbose=False)
            ...
            if loss + opt.es_min_delta < best_loss:
                best_loss = loss
                best_epoch = epoch
                print('{} Best Saved Epoch : {}'.format(str(datetime.now())[:19], best_epoch))
